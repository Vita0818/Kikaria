...
enum KikariaLocalization {
    ...
    static func dateFormatter(_ format: String) -> DateFormatter {
        ...
        formatter.locale = Locale(identifier: usesEnglish ? "en_US_POSIX" : "zh_Hans_CN")
    ...
    static func daysLeftText(days: Int?) -> String {
        let value = days.map(String.init) ?? "--"
        return usesEnglish ? "\(value) left" : "剩余 \(value) 天"
    }

    static func goalUnit(_ count: Int) -> String {
        usesEnglish ? "\(count)" : "\(count) 个"
    }

    static func knowledgePointCount(_ count: Int) -> String {
        usesEnglish ? "\(count) pts" : "\(count) 个知识点"
    }

    static func recordCount(_ count: Int) -> String {
        usesEnglish ? "\(count) records" : "\(count) 条记录"
    }

    static func selectedTagsSummary(isEmpty: Bool, count: Int) -> String {
        if usesEnglish {
    ...
    static func todayReviewCount(_ count: Int) -> String {
        usesEnglish ? "\(count) reviews today" : "该知识点今日复习 \(count) 次"
    }

    static func progressMessage(markedMasteredCount: Int, reviewedAnswerCount: Int, dailyGoal: Int) -> String {
        if usesEnglish {
            ...
            if reviewedAnswerCount > 0 {
                return "\(max(0, dailyGoal - markedMasteredCount)) more to goal."
            }
        ...
        if reviewedAnswerCount > 0 {
            return "今日已经进入状态，还差 \(max(0, dailyGoal - markedMasteredCount)) 个新增掌握达到目标。"
        }

        return "今天还很安静，可以从一个知识点开始。"
    }

    static func notificationBody(presetName: String) -> String {
        usesEnglish
            ? "Today's \(presetName) goal is still behind."
            : "今天的「\(presetName)」学习量尚未达标哦，抓紧学习吧！"
    }

    static func presetSwitchedToast(_ presetName: String) -> String {
        usesEnglish ? "Switched to \(presetName)" : "已切换至「\(presetName)」"
    }

    static func presetCreatedToast(_ presetName: String) -> String {
        usesEnglish ? "Created \(presetName)" : "已创建「\(presetName)」"
    }

    static func presetDeletedToast(_ presetName: String) -> String {
        usesEnglish ? "Deleted \(presetName)" : "已删除「\(presetName)」"
    }

    static func pointsUpdatedToast(_ count: Int) -> String {
        usesEnglish ? "Updated \(count) points" : "已更新 \(count) 个知识点"
    }

    static func masteredToast(_ title: String) -> String {
        usesEnglish ? "\(title) mastered" : "\(title) 已掌握"
    }

    static func removedFocusToast(_ title: String) -> String {
        usesEnglish ? "\(title) removed from Focus" : "\(title) 已移出重点集锦"
    }

    static func removedMasteredToast(_ title: String) -> String {
        usesEnglish ? "\(title) removed from Mastered" : "\(title) 已移出已掌握"
    }

    static func addedFocusToast(title: String, count: Int) -> String {
        if usesEnglish {
    ...
    static func addFocusButtonTitle(count: Int) -> String {
        if usesEnglish {
    ...
    static func builtInPresetDisplayName(_ name: String, isBuiltIn: Bool) -> String {
        guard isBuiltIn, usesEnglish else {
            return name
        }

        return builtInPresetNames[name] ?? name
    }

    static var weekdaySymbols: [String] {
        usesEnglish ? ["M", "T", "W", "T", "F", "S", "S"] : ["一", "二", "三", "四", "五", "六", "日"]
    }

    static func monthTitle(for date: Date) -> String {
        let format = usesEnglish ? "MMM yyyy" : "yyyy年 M月"
        return dateFormatter(format).string(from: date)
    }

    static func monthDayTitle(for date: Date) -> String {
        let format = usesEnglish ? "MMM d" : "M月d日"
        return dateFormatter(format).string(from: date)
    }

    static func homeDateTitle(for date: Date) -> String {
        if usesEnglish {
            let formatter = dateFormatter("MMM")
            let day = Calendar.current.component(.day, from: date)
            return "\(formatter.string(from: date)) \(day)\(ordinalSuffix(for: day))"
        }

        return dateFormatter("M月d日").string(from: date)
    }

    static var markdownFormatTemplate: String {
        if usesEnglish {
            return """
            # Point title
    ...
    static var markdownCompleteExample: String {
        if usesEnglish {
            return """
    ...
    static var markdownLatexExample: String {
        if usesEnglish {
            return """
    ...
    static var markdownAIPrompt: String {
        if usesEnglish {
            return """
    ...
    private static let builtInPresetNames: [String: String] = [
        "大学物理": "Physics",
...
enum KikariaTypography {
    ...
    static func localizedSerifText(_ string: String, size: CGFloat, weight: Font.Weight = .regular) -> Text {
        serifText(KikariaLocalization.string(string), size: size, weight: weight)
    }
    ...
    static func localizedMixedText(_ string: String, size: CGFloat, weight: Font.Weight = .regular) -> Text {
        mixedText(KikariaLocalization.string(string), size: size, weight: weight)
    }
